				...
				for (int i = 0; i < sampleLength; i += bufferLength) 
				{
					...
					for (int i2 = 0; i2 < bufferLength; i2++) 
					{
						...
						if (nbChannels == 2) {
							...
							finalSample[1][finalI] = valuesR[i2];
						}
						finalI++;
						currentByte = valueToFrame(stereoValue);
						destinationFile.write(currentByte);
					}
				}
				
				// currentWavFile.setSamplesFromDouble(finalSample);
				// currentWavFile.writeToFilePath(destination.getAbsolutePath());
